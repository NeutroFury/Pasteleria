...
export default function Productos() {
  ...
  useEffect(() => {
    const load = () => setProductos(productService.getAll());
    load();
  ...
  const agregarAlCarrito = async (p) => {
    // if logged in, try to call backend by productId (prefer p.id)
    const token = localStorage.getItem('token');
    if (token) {
      try {
        let productId = p.id || p.productId || null;
        if (!productId) {
          // try to resolve via productService by codigo
          const found = await productService.getById(p.codigo);
          productId = found?.id || found?.codigo || null;
        }
        if (productId) {
          await api.addToCartByProductId(productId, 1);
          // update cart count by dispatching carrito-changed so Header reloads
    ...
    window.ProductosLogic.agregarAlCarrito(p.codigo, productos);
  };
